...
//       return true;
////        int indexSpecialChar = password.indexOf('!');
//        //int indexAt = email.indexOf('@');
//    }
    //===========================================================
private boolean isPasswordValid(String password) {
    // 1. Должна присутствовать @
    int indexAt = password.indexOf('@');
    if (indexAt == -1 || indexAt != password.lastIndexOf('@')) return false;

    // 2. Точка после собаки
    if (password.indexOf('.', indexAt + 2) == -1) return false;

    // 3. После последней точки должно быть минимум 2 символа
    if (password.lastIndexOf('.') >= password.length() - 2) return false;
    ...
    if (!Character.isAlphabetic(password.charAt(0))) return false;
